...
NEXTLB1INSTANCE=`curl -H X_API_VERSION:1.5 -b ~/mycookie -X GET https://us-3.rightscale.com$LB1 |python -mjson.tool |grep instances|cut -c46-58|tr -d '\r'`

LB2=`egrep Location output/08_Servers-Clone.sh.out |cut -c 11-|tr -d '\r' |sed -n 1p`
NEXTLB2INSTANCE=`curl -H X_API_VERSION:1.5 -b ~/mycookie -X GET https://us-3.rightscale.com$LB2 |python -mjson.tool |grep instances|cut -c46-58|tr -d '\r'`
