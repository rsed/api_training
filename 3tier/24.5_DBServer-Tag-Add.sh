...
CURRDBINSTANCE=`curl -H X-API-Version:1.5 -b ~/mycookie -X GET https://us-3.rightscale.com$DBSERVERHREF |python -mjson.tool | grep instances |sed -n 1p | awk '{split($2,a,"\""); print a[2]}' | tr -d '\r'`
